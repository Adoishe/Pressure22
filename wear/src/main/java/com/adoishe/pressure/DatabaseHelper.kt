...
class DatabaseHelperOLD(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
 ...
 */
        val selectQuery         =
            //,'localtime'
            "SELECT  * FROM $TABLE_PRESSURES where datetime($KEY_PERIOD) BETWEEN datetime('now','$countHour hours') and datetime('now')  order by period desc LIMIT 1"
    ...
    fun addPressure(pressure: Double): Long {
        ...
        var period = System.currentTimeMillis()
